...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Enforce authentication only for protected recordings
    ...
    if (resource.storage_provider === "supabase" && resource.storage_path) {
      const signed = await adminClient.storage.from("sacred-circle-media").createSignedUrl(resource.storage_path, 60 * 15);
